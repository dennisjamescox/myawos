...
logfilepath = "/home/pi/adaclick/ramdisk/"
...
print("current_time,", "sample_no,", "round(mean),", "round(PEAK),", "low_sample_no,","LOW1,", "high_sample_no,", "clickcount,", "HIT,", "round(max_mean),", "round(TRIGGERLEVEL)", file = l)
...
#       print("lastvalues[", i, "] =", lastvalues[i])
    ...
    print(f'{current_time}, {sample_no:05}, {round(mean):05}, {round(PEAK):05}, {low_sample_no}, {LOW1}, {high_sample_no}, {clickcount}, {HIT}, {round(max_mean):05}, {round(TRIGGERLEVEL):05}',  file = l)
    flushcount +=1
...
#       print(current_time, "LOW recognized", file = l)
    if LOW1 == 1:
       if mean > TRIGGERLEVEL:   # scanning for rising edge...
          ...
          if adaptTRGlevel == 0:
             adaptTRGlevel = 1
             TRIGGERLEVEL = max_mean * 0.9 # !!!! Icom Handheld: absolute level of click falls click by click !!!!
...
#          print(f'{current_time}, {round(mean):06}, {sample_no:06}, {LOW1}, {high_sample_no}, {HIT}, {clickcount}',  file = l)
    if LOW1 == 1 and high_sample_no > 0:    # rising edge detected, scanning for falling edge...
          if mean < TRIGGERLEVEL:
             LOW2 = 1
#             current_time = datetime.utcnow().strftime("%Y%m%d-%H:%M:%S.%f")
#             print(current_time, mean, "LOW2 recognized", file = l)
    ...
    #   sample_no = 0
       max_mean = 0
    #   adaptTRGlevel = 0    # at the end of 5 sec cycle...
    if flushcount == 10:
       l.flush()
       flushcount = 0
